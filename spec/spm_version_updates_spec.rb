  ...
  describe Danger::DangerSpmVersionUpdates do
    ...
    describe "with Dangerfile" do
      ...
      it "(local) Does report for revision when configured: bad input and new version" do
        allow(@my_plugin).to receive_messages(filter_remote_packages: [],
                                              git_versions: [
      ...
      it "(local) Does not report new versions for exact when configured" do
        allow(@my_plugin).to receive_messages(filter_remote_packages: [],
                                              git_versions: [
                                                Semantic::Version.new("3.5.0"),
      ...
      it "(local) Does report new commit for branch when configured" do
        allow(@my_plugin).to receive_messages(filter_remote_packages: [], git_branch_last_commit: "NEWER_COMMIT_HASH")
      ...
      it "(local) Does not report for branch" do
        allow(@my_plugin).to receive_messages(filter_remote_packages: [], git_branch_last_commit: "abfb06df71c287873dd96d478ef5ce185c54414b")
